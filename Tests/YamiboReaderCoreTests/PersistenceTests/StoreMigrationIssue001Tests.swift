...
struct GRDBStoreMigrationIssue001Tests {
    ...
    @Test func genericCacheStoresJSONBodyOnDiskAndThinMetadataInDatabase() async throws {
        ...
        let store = GRDBJSONCacheStore(writer: pool, rootDirectory: root)
    ...
    @Test func readsDoNotExtendTTLAndOnlyUpdateLRUAccessTime() async throws {
        ...
        let store = GRDBJSONCacheStore(writer: pool, rootDirectory: root, now: { now })
