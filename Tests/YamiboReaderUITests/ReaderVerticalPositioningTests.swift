...
final class ReaderVerticalPositioningTests: XCTestCase {
    ...
    func testReaderContainerViewDoesNotUseFrameSamplerForVerticalTextPosition() throws {
        let source = try String(contentsOfFile: projectFilePath("Sources/YamiboReaderUI/Features/Reader/NovelReader/Container/ReaderContainerView.swift"))
    ...
    func testSheetPresentationChangesOnlyUpdateChrome() throws {
        let source = try String(contentsOfFile: projectFilePath("Sources/YamiboReaderUI/Features/Reader/NovelReader/Container/ReaderContainerPresentationModifiers.swift"))
        let body = try functionBody(
