...
final class MangaReaderDirectoryPanelTests: XCTestCase {
    func testReaderViewWiresFullscreenControlsSheetAndViewportPlacement() throws {
        ...
        XCTAssertTrue(source.contains("ReaderBottomActionRowPresentation(isScrubbing: activeVerticalProgressPreview != nil)"))
        XCTAssertTrue(source.contains(".opacity(controlVisibility.opacity)"))
        XCTAssertTrue(source.contains(".allowsHitTesting(controlVisibility.allowsHitTesting)"))
        XCTAssertTrue(source.contains("if let preview = activeVerticalProgressPreview"))
