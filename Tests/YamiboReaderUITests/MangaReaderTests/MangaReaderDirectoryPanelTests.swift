...
final class MangaReaderDirectoryPanelTests: XCTestCase {
    func testReaderViewWiresFullscreenControlsSheetAndViewportPlacement() throws {
        ...
        XCTAssertTrue(source.contains("imagePipeline: model.imagePipeline"))
        XCTAssertTrue(source.contains("isChromeVisible: isChromeVisible"))
        XCTAssertTrue(source.contains("let usesTwoPageSpread = MangaPagedLayoutPolicy.usesTwoPageSpread("))
        XCTAssertTrue(source.contains("summary: mangaChromeSummary("))
        XCTAssertTrue(source.contains("usesTwoPageSpread: usesTwoPageSpread"))
        XCTAssertTrue(source.contains("readingMode: model.presentation.settings.readingMode"))
        XCTAssertTrue(source.contains("pageTurnDirection: model.presentation.settings.pageTurnDirection"))
        XCTAssertTrue(source.contains("struct MangaReaderChromeSummary"))
        XCTAssertTrue(source.contains("pagePreviewTargets: [Int: MangaReaderPageProjection]"))
        XCTAssertTrue(source.contains("let pagePreviewTargets = pages.reduce(into: [Int: MangaReaderPageProjection]())"))
        XCTAssertTrue(source.contains("guard page.tid == currentPage.tid else { return }"))
        XCTAssertTrue(source.contains("result[page.localIndex] = page"))
        ...
        XCTAssertTrue(source.contains("L10n.string(\"manga.preview_page_label\", pageLabel, itemCount)"))
        ...
        XCTAssertTrue(source.contains("progressChromePresentation.showsVerticalScrubber"))
        XCTAssertTrue(source.contains("ReaderProgressChromePresentation(\n            readingMode: readingMode.readerChromeReadingMode,\n            isChromeVisible: true\n        )"))
        XCTAssertTrue(source.contains("fillDirection: pageTurnDirection.progressFillDirection"))
        XCTAssertTrue(source.contains("showsFill: progressChromePresentation.showsHorizontalFill"))
        XCTAssertTrue(source.contains("supportsScrub: progressChromePresentation.supportsHorizontalScrub && sliderHasAvailableRange"))
        XCTAssertTrue(source.contains("onJumpToLocalPage(target)"))
        XCTAssertTrue(source.contains("previewSize: MangaReaderProgressImagePreview.previewSize"))
        XCTAssertTrue(source.contains("showsPreview: false"))
        XCTAssertTrue(source.contains("onPreviewChange: { activeVerticalProgressPreview = $0 }"))
        XCTAssertTrue(source.contains("EmptyView()"))
        XCTAssertTrue(source.contains("MangaReaderProgressImagePreview("))
        XCTAssertTrue(source.contains("@State private var horizontalScrubState = ReaderProgressScrubState()"))
        XCTAssertTrue(source.contains("@State private var activeVerticalProgressPreview: ReaderProgressScrubPreview?"))
        XCTAssertTrue(source.contains("let verticalScrubVisibility = ReaderBottomActionRowPresentation(isScrubbing: activeVerticalProgressPreview != nil)"))
        XCTAssertTrue(source.contains("isScrubbing: horizontalScrubState.phase == .scrubbing || activeVerticalProgressPreview != nil"))
        XCTAssertTrue(source.contains("let centerProgressPreview = progressChromePresentation.showsVerticalScrubber"))
        XCTAssertTrue(source.contains(": horizontalScrubState.preview"))
        XCTAssertTrue(source.contains(".opacity(staticControlVisibility.opacity)"))
        XCTAssertTrue(source.contains(".allowsHitTesting(staticControlVisibility.allowsHitTesting)"))
        XCTAssertTrue(source.contains(".opacity(verticalScrubVisibility.opacity)"))
        XCTAssertTrue(source.contains(".allowsHitTesting(verticalScrubVisibility.allowsHitTesting)"))
        XCTAssertTrue(source.contains("if let preview = centerProgressPreview"))
        XCTAssertTrue(source.contains("page: summary?.pagePreviewTargets[preview.targetIndex]"))
        XCTAssertTrue(source.contains("private extension MangaPageTurnDirection"))
        XCTAssertTrue(source.contains("case .rightToLeft:\n            .rightToLeft"))
        ...
        XCTAssertTrue(source.contains("max(bottomInset - 18, 8)"))
        XCTAssertTrue(source.contains("private func bottomActionButton("))
        XCTAssertTrue(source.contains("layout.actionButtonIconFrame"))
        XCTAssertTrue(source.contains(".readerChromeButtonStyle(tint: readerChromeButtonTint(for: colorScheme))"))
