...
struct MangaReaderPresentationInfrastructureTests {
    ...
    @Test func pageCurlViewportConsumesHiddenPhysicalSurfaceEdgesBeforePageTurn() throws {
        ...
        #expect(pageCurlSource.contains("isZoomActive: MangaPageZoomPolicy.isActive(pageCurlZoomScale)"))
        #expect(pageCurlSource.contains("currentPageCurlSurfaceInteraction(in: pageViewController)"))
        #expect(pageCurlSource.contains("isZoomActive: surfaceInteraction.isZoomActive"))
    ...
    @Test func pageCurlSinglePageScopesTransientZoomToPageAppearance() throws {
        let source = try sourceFile("Sources/YamiboReaderUI/Features/Reader/MangaReader/Presentation/MangaPagedReaderViewport.swift")
        let pageCurlRange = try #require(source.range(of: "struct MangaPagedPageCurlReaderViewport"))
        let surfaceModelRange = try #require(source.range(of: "private struct MangaPagedReaderSpreadPageSurface"))
        let setSelectionRange = try #require(source.range(of: "private func setSelection("))
        let navigationDirectionRange = try #require(source.range(of: "private func navigationDirection"))
        let pageSlotRange = try #require(source.range(of: "private struct MangaPagedReaderPageSlot"))
        let zoomableSpreadRange = try #require(source.range(of: "private struct MangaPagedReaderZoomableSpreadSurface"))
        let pageSurfaceRange = try #require(source.range(of: "private struct MangaPagedReaderPageSurface: View"))
        ...
        let setSelectionSource = String(source[setSelectionRange.lowerBound..<navigationDirectionRange.lowerBound])
        let surfaceModelSource = String(source[surfaceModelRange.lowerBound..<pageSlotRange.lowerBound])
        let pageSlotSource = String(source[pageSlotRange.lowerBound..<zoomableSpreadRange.lowerBound])
        let pageSurfaceSource = String(source[pageSurfaceRange.lowerBound..<scaledImageRange.lowerBound])
        let scaledImageSource = String(source[scaledImageRange.lowerBound..<edgeFillRange.lowerBound])

        #expect(pageCurlSource.contains("private var pageCurlPageAppearanceGenerations: [String: Int] = [:]"))
        #expect(pageCurlSource.contains("preparePreviousPageCurlPagesForReuse(previousViewControllers)"))
        #expect(pageCurlSource.contains("private func preparePreviousPageCurlPagesForReuse(_ previousViewControllers: [UIViewController])"))
        #expect(pageCurlSource.contains("guard !parent.sequence.usesTwoPageSpread else { return }"))
        #expect(pageCurlSource.contains("pageCurlPageAppearanceGenerations[page.id, default: 0] += 1"))
        ...
        #expect(pageCurlSource.contains("pageCurlPageSurfaceIdentity(for: page)"))
        #expect(setSelectionSource.contains("let outgoingViewControllers = pageViewController.viewControllers ?? []"))
        #expect(setSelectionSource.contains("let shouldPrepareOutgoingPageCurlPages = !parent.sequence.usesTwoPageSpread &&\n                clampedSelectionIndex != currentSelectionIndex"))
        #expect(setSelectionSource.contains("if animated, shouldPrepareOutgoingPageCurlPages {\n                    self.preparePreviousPageCurlPagesForReuse(outgoingViewControllers)\n                }"))
        #expect(setSelectionSource.contains("if shouldPrepareOutgoingPageCurlPages {\n                    preparePreviousPageCurlPagesForReuse(outgoingViewControllers)\n                }\n                currentSelectionIndex = clampedSelectionIndex"))
        #expect(surfaceModelSource.contains("let surfaceIdentity: MangaPagedReaderPageAppearanceIdentity"))
        #expect(surfaceModelSource.contains("private struct MangaPagedReaderPageAppearanceIdentity: Hashable"))
        #expect(pageSlotSource.contains("surfaceIdentity: surface.surfaceIdentity"))
        #expect(pageSurfaceSource.contains(".id(surfaceIdentity)"))
        #expect(scaledImageSource.contains("@State private var steadyScale: CGFloat = 1"))
        #expect(!source.contains("zoomResetRequest"))
    ...
    @Test func pagedViewportQuickFadePanDefersToPhysicalHiddenSurfaceEdges() throws {
        ...
        #expect(deferSource.contains("isZoomActive: surfaceInteraction.isZoomActive"))
        ...
        #expect(interactionSource.contains("private(set) var isZoomActive = false"))
        #expect(interactionSource.contains("func updateZoomActive(_ isZoomActive: Bool)"))
    ...
    @Test func pagedViewportDefersSlidePanToUnzoomedSurfaceWhenHiddenContentRemains() throws {
        ...
        #expect(source.contains("isZoomActive: surfaceInteraction.isZoomActive"))
    ...
    @Test func pagedViewportGatesZoomAndSurfacePanBehindHiddenChrome() throws {
        ...
        #expect(source.contains("let edgeRevealRequests = PassthroughSubject<MangaPagedReaderEdgeRevealRequest, Never>()"))
        #expect(source.contains("let zoomToggleRequests = PassthroughSubject<MangaPagedReaderZoomToggleRequest, Never>()"))
        #expect(source.contains("func requestZoomToggle(at location: CGPoint)"))
        #expect(source.contains(".onReceive(surfaceInteraction.edgeRevealRequests)"))
        #expect(source.contains(".onReceive(surfaceInteraction.zoomToggleRequests)"))
        #expect(source.contains(".onReceive(spreadSurfaceInteraction.edgeRevealRequests)"))
        #expect(source.contains(".onReceive(spreadSurfaceInteraction.zoomToggleRequests)"))
        #expect(!source.contains("@Published private(set) var edgeRevealRequest"))
        #expect(!source.contains("@Published private(set) var zoomToggleRequest"))
        #expect(!source.contains("$edgeRevealRequest"))
        #expect(!source.contains("$zoomToggleRequest"))
        ...
        #expect(source.contains("let isSurfaceZoomActive = isZoomInteractionEnabled && MangaPageZoomPolicy.isActive(zoomScale)"))
        #expect(source.contains(".onChange(of: isSurfaceZoomActive, initial: true)"))
        #expect(source.contains("spreadSurfaceInteraction.updateZoomActive(newValue)"))
        #expect(source.contains("surfaceInteraction.updateZoomActive(newValue)"))
        #expect(source.contains("spreadSurfaceInteraction.updateZoomActive(false)"))
        #expect(source.contains("surfaceInteraction.updateZoomActive(false)"))
