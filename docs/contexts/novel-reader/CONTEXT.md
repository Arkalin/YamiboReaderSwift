...
_Avoid_: enqueue time, failure time, transparent projection cache time
...
- A forum **Thread Page** is the authoritative source model for novel-thread content that can be reused by native novel reading. A **Novel Reader Projection** may be cached for layout and transparent performance reuse, but it is derived from a **Thread Page** rather than being a second source of truth.
...
- A **Novel Offline Cache Entry** is created or refreshed only by user cache intent. Successful ordinary online reading may update transparent **Thread Page** and `NovelReaderProjectionStore` projection caches, but it must not silently create user-retained offline content.
...
- A **Novel Offline Cache Entry** owns a durable **Novel Offline Source Page** as its authoritative offline content, its **Novel Offline Update Time**, and any user-retained **Novel Offline Image Asset** records acquired for that entry. **Novel Reader Projection** JSON lives in `NovelReaderProjectionStore` as transparent, regenerable performance cache, not as offline-cache entry payload.
...
- Refreshing a **Novel Offline Cache Entry** atomically replaces its **Novel Offline Source Page** only after the new source page is durable. Refresh failure preserves the previous offline-readable content, and transparent projection cache refresh remains nonfatal.
- Deleting a **Novel Offline Cache Entry** removes user-retained offline membership, source page, and retained offline image assets for that entry without clearing transparent **Thread Page** or **Novel Reader Projection** caches.
...
- When **Novel Offline Fallback** finds a missing or stale transparent **Novel Reader Projection** in `NovelReaderProjectionStore`, it derives one from the **Novel Offline Source Page** and may write the refreshed projection back as a nonfatal transparent performance cache update.
...
- The live runtime targets the iOS novel reader; UIKit is the production TextKit 2 Runtime Adapter, while the test Runtime Adapter exercises the same package-internal Interface.
...
- A new runtime generation must build and validate its authoritative **Novel Text Viewport Index** from its own candidate TextKit 2 object graph; only semantic pure-value inputs and offset maps may be reused across generations.
...
- Every external block occupies its own page or vertical chunk surface; text before and after it belongs to separate text surfaces, and one surface does not mix TextKit fragments with external block content.
...
- Chapter ordinal, chapter title, page number, and intra-page progress are presentation metadata and are never authoritative restoration identity.
...
- Production **Novel Text Layout** derives page ranges and chunk ranges from the complete composed **Novel Text Viewport** document through TextKit 2; estimated text slicing does not exist and must not return as a layout source, even for tests.
...
- Reader page document encoding writes the explicit chapter structure and carries a schema version. A cached document with an unsupported schema version is invalid cache content and is re-derived from the authoritative author-scoped **Thread Page**; there is no partial decoding or lazy schema upgrade path.
- Corrupt segment content or an out-of-range explicit title range invalidates the cached document and requires source reload.
...
- Text measurement belongs to the same **Novel Text Viewport** creation and update path; callers must not use a separate text-height measurement Interface such as measuring a rendered text value directly.
...
- A **Novel Reader Surface Identity** is a pure `Hashable` and `Sendable` value containing opaque runtime generation and surface ordinal. Presentation values carry surface identities rather than live display references or bare page indexes.
...
- A **Novel Text Viewport** display reference is a caller-isolated opaque reference containing only a weak runtime-owner reference and one **Novel Reader Surface Identity**.
...
- One opaque prepared transaction, confined to the workflow's isolation domain, owns the candidate TextKit graph, generation-scoped Index and frozen geometry, committed settings and container projection, current reader page document, and the Session-prepared revision-zero **Novel Reader Presentation**.
...
- Commit is one synchronous operation with no suspension in the workflow's isolation domain: it activates the candidate graph, replaces Session state, and publishes the Workflow Presentation together. SwiftUI must never observe a new Index with the old graph or new settings with the old Presentation.
...
- HTML text transformation, chapter annotation, and semantic attributed-document preparation may run outside the workflow's isolation domain when their outputs remain pure-value and `Sendable`.
...
- Creating or mutating `NSTextContentStorage`, `NSTextLayoutManager`, TextKit layout fragments, the complete **Novel Text Viewport Index**, and the committed runtime generation occurs inside the synchronous runtime transaction in the workflow's isolation domain.
- TextKit materialization resolves actual platform fonts and fallbacks, builds the platform attributed document, creates the candidate graph, builds the Index, and freezes geometry.
...
- Live TextKit runtimes are not retained in an LRU across reader page documents; only semantic pure-value inputs and offset maps may be cached across documents.
...
- Text and external-block surfaces hold opaque display references identified by **Novel Reader Surface Identity**; they do not hold rendered text values or live TextKit objects.
...
- Core tests cover atomic commit, failed-update rollback, latest-wins cancellation, generation invalidation, and prefetched-document promotion.
...
- Runtime signposts may measure semantic preparation, TextKit indexing, commit, viewport update, and drawing duration, but device-specific elapsed-time thresholds are performance observations rather than correctness requirements.
- The **Novel Reading Session** remains pure-value and `Sendable`, with no TextKit or platform UI imports; UI code owns no text measurement, per-page TextKit object graphs, or TextKit cache invalidation.
...
- Rendered-page compatibility shapes no longer exist. Anything a platform adapter consumes is derived one-way from a committed **Novel Text Layout** result and must never become an input to the **Novel Reading Session**, runtime owner, **Novel Text Viewport Index**, measurement, or drawing.
...
- Do not reintroduce a standalone paginator shim, text view fitting, or estimated slicing as a production alternative to **Novel Text Layout**.
