...
- A **Novel Offline Cache Entry** is created or refreshed only by user cache intent. Successful ordinary online reading may update transparent **Thread Page** and `ReaderCacheStore` projection caches, but it must not silently create user-retained offline content.
...
- A **Novel Offline Cache Entry** owns a durable **Novel Offline Source Page** as its authoritative offline content, its **Novel Offline Update Time**, and any user-retained **Novel Offline Image Asset** records acquired for that entry. **Novel Reader Projection** JSON lives in `ReaderCacheStore` as transparent, regenerable performance cache, not as offline-cache entry payload.
...
- When **Novel Offline Fallback** finds a missing or stale transparent **Novel Reader Projection** in `ReaderCacheStore`, it derives one from the **Novel Offline Source Page** and may write the refreshed projection back as a nonfatal transparent performance cache update.
