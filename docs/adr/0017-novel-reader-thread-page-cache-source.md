...
Native novel reading uses the author-scoped forum **Thread Page** cache as the authoritative cached source for novel-thread content. The persisted **Novel Reader Projection** is retained only as a derived transparent performance cache for reader layout, chapter identity, segment identity, and image references; it must be validated against the corresponding **Thread Page** before reuse.
