...
- `Sources/YamiboXCore` is organized by feature (`Account`, `Forum`, `History`, `Library`, `Like`, `Reader/{Manga,Novel,Shared}`, `Settings`, `Sync`, `Update`) with `Domain`/`Application`/`Data` layers inside each feature. Cross-cutting infrastructure lives under `Sources/YamiboXCore/Infrastructure/{Networking,HTML,Images,Localization,Logging}`; the cross-feature thread routing/dispatch module lives under `Sources/YamiboXCore/Routing/` (it knows Forum/Manga/Novel launch contexts, so it is deliberately not `Infrastructure`); the shared GRDB database lifecycle lives under `Sources/YamiboXCore/Persistence/`, with each feature contributing its own schema module. Core is UIKit-free and WebKit-free (web-view data clearing is injected through `WebsiteDataClearing`).
...
- `Sources/YamiboXUI` mirrors the feature layout under `Features/` (`Favorites`, `Forum`, `History`, `ImageBrowser`, `Like`, `Mine`, `OfflineCache`, `Reader`, `Settings`); cross-feature UI helpers live under `Sources/YamiboXUI/Platform/`. Favorites' non-view machinery (organizer, quick actions, the shared `FavoriteActionController`) lives in `Features/Favorites/Engine/`; the offline-cache download-queue screens and their view model live in `Features/OfflineCache/` and are shared by the Mine tab and both readers' cache sheets.
- Forum UI is clustered by screen under `Features/Forum/` (`Navigation`, `Home`, `Board`, `Search`, `Detail`, `Blog`, `Messages`, `Web`, `Shared`, `ThreadReader`, `UserSpace`); every destination is instantiated by `Navigation/ForumDestinationScreen.swift`, and `Navigation/ForumDestinationNavigator.swift` owns routing decisions.
